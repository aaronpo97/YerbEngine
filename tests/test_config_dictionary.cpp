...
BOOST_AUTO_TEST_CASE(test_set_get_various_types) {
...

    {
        ...
        config.set<SDL_Color>("background_color",
...

    {
        ...
        BOOST_CHECK_EQUAL(bgColor.value().r, 0xFF);
        BOOST_CHECK_EQUAL(bgColor.value().g, 0xFF);
        BOOST_CHECK_EQUAL(bgColor.value().b, 0xFF);
        BOOST_CHECK_EQUAL(bgColor.value().a, 0xFF);
    }
}
}