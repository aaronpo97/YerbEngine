...
protected:
   ...
   *
   * This function is responsible for calling the user input system and updating
   * the game engine.
   *
   ...
   *
   * The game engine listens for SDL events such as key presses, mouse clicks,
   * and window events.
   *
   * If the current active scene uses the event as a Scene Action, the action is
   * triggered.
   *
   */
  void sUserInput();
  ...
  createConfigManager(const Path &configPath);

  /**
   * Create the TextureManager object.
   *
   * The TextureManager is responsible the allocated memory for textures and
   * surfaces..
   *
   * When the TextureManager is created, it loads all surfaces from the assets
   * folder and stores them in memory and generates textures from the surfaces.
   *
   * When it goes out of scope, the textures and surfaces are automatically
   * deleted.
   *
   * @returns std::unique_ptr<TextureManager> The initialized TextureManager
   * object.
   */
   ...
   *
   * The video manager is responsible for managing the SDL window and renderer.
   * It uses the ConfigManager object to get the window size and other related
   * configurations.
   *
   * When the video manager is created, the window and renderer are created and
   * initialized. When it goes out of scope, the window and renderer are
   * destroyed.
   *
   ...
   *
   * The AudioManager is responsible for managing the audio system in the game
   * engine.
   *
   ...
   *
   * The AudioSampleQueue's role in the game engine is to manage the queue of
   * audio samples to be played. It uses the AudioManager to play the audio
   * samples.
   *
...
public:
   ...
   *
   * If the game engine is running in a web browser, it uses the emscripten main
   * loop. Otherwise it simply uses a while loop.
   */
  void run();
