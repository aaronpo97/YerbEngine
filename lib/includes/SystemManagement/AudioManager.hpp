...
namespace YerbEngine {
    ...
    namespace AudioPath {
        Path const ASSETS  = "assets/audio";
        Path const TRACKS  = ASSETS / "tracks";
        Path const SAMPLES = ASSETS / "samples";

        Path const MAIN_MENU = TRACKS / "main_menu.ogg";
        Path const PLAY      = TRACKS / "play.ogg";

        Path const ITEM_ACQUIRED   = SAMPLES / "item_acquired.wav";
        Path const SPEED_BOOST     = SAMPLES / "speed_boost_acquired.wav";
        Path const ENEMY_COLLISION = SAMPLES / "enemy_collides.wav";
        Path const MENU_MOVE       = SAMPLES / "menu_move.wav";
        Path const MENU_SELECT     = SAMPLES / "menu_select.wav";
        Path const SHOOT           = SAMPLES / "shoot.wav";
        Path const BULLET_HIT_01   = SAMPLES / "bullet_hit_01.wav";
        Path const BULLET_HIT_02   = SAMPLES / "bullet_hit_02.wav";
        Path const SLOWNESS_DEBUFF = SAMPLES / "slowness_debuff_acquired.wav";
    } // namespace AudioPath
    ...
    class AudioManager {
         ...
         */
        void loadSample(AudioSample sample,
                        Path const &filepath);
      ...
      public:
         ...
         * AudioManager. It uses the loadTrack() and loadSample() methods to
         * load each track and sample.
         */
        void loadAllAudio();
         ...
         * @param volume The volume level (0 to MIX_MAX_VOLUME).
         */
        void setSampleVolume(AudioSample sampleTag,
                             int         volume);
