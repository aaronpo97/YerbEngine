...
namespace YerbEngine {
    ...
    void VideoManager::initializeVideoSystem() {
        if (SDL_Init(SDL_INIT_VIDEO) != 0) {
            SDL_LogError(SDL_LOG_CATEGORY_VIDEO,
                         "SDL video system is not ready: %s", SDL_GetError());
    ...
    void VideoManager::updateWindowSize() {
        ...
        m_configManager.updateGameWindowSize(
            Vec2{static_cast<float>(currentWindowWidth),
                 static_cast<float>(currentWindowHeight)});
    }
    ...
    void VideoManager::setupRenderer() const {
        ...
        constexpr SDL_Color backgroundColor = {
            .r = 0, .g = 0, .b = 0, .a = 255};
        SDL_SetRenderDrawColor(m_renderer, backgroundColor.r, backgroundColor.g,
    ...
    Vec2 VideoManager::getWindowSize() const { return m_currentWindowSize; }

    SDL_Renderer *VideoManager::getRenderer() const { return m_renderer; }

    SDL_Window *VideoManager::getWindow() const { return m_window; }
    ...
    VideoManager::~VideoManager() { cleanup(); }
