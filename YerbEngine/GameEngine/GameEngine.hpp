...
class GameEngine {
  protected:
     ...
     * and window events.
     *
     * If the current active scene uses the event as a Scene Action, the action
     * is triggered.
     *
     */
    void sUserInput();
     ...
     * folder and stores them in memory and generates textures from the
     * surfaces.
     *
     ...
     *
     * When the video manager is created, the window and renderer are created
     * and initialized. When it goes out of scope, the window and renderer are
     * destroyed.
     *
  ...
  public:
    /**
     * Constructs the GameEngine object and initializes all necessary managers
     * and resources.
     *
     ...
     *
     * If the game engine is running in a web browser, it uses the emscripten
     * main loop. Otherwise it simply uses a while loop.
     */
    void run();
