...
void HowToPlayScene::renderText() const {
  ...
  TextHelpers::renderLineOfText(
          renderer, fontMd, controlsText, textColor, controlsPos);
  ...
  TextHelpers::renderLineOfText(
          renderer, fontSm, enterText, textColor, enterPos);
  ...
  TextHelpers::renderLineOfText(
          renderer, fontSm, shootText, textColor, shootPos);
  ...
  TextHelpers::renderLineOfText(
          renderer, fontMd, objectivesText, textColor, objectivesPos);

  const std::string objective1Text =
          "1. Collect the yellow squares to increase your score.";
  const Vec2 objective1Pos = objectivesPos + Vec2{0, 80};
  TextHelpers::renderLineOfText(
          renderer, fontSm, objective1Text, textColor, objective1Pos);
  ...
  TextHelpers::renderLineOfText(
          renderer, fontSm, objective2Text, textColor, objective2Pos);

  const std::string objective3Text =
          "3. Collect the green squares to gain a speed boost.";
  const Vec2 objective3Pos = objective2Pos + Vec2{0, 40};
  TextHelpers::renderLineOfText(
          renderer, fontSm, objective3Text, textColor, objective3Pos);

  const std::string objective4Text =
          "4. Avoid purple squares as they will slow you down.";
  const Vec2 objective4Pos = objective3Pos + Vec2{0, 40};
  TextHelpers::renderLineOfText(
          renderer, fontSm, objective4Text, textColor, objective4Pos);

  const std::string objective5Text =
          "5. Shoot down red squares to increase your score.";
  const Vec2 objective5Pos = objective4Pos + Vec2{0, 40};
  TextHelpers::renderLineOfText(
          renderer, fontSm, objective5Text, textColor, objective5Pos);

  const std::string objective6Text =
          "6. Avoid shooting any squares you want to collect.";
  const Vec2 objective6Pos = objective5Pos + Vec2{0, 40};
  TextHelpers::renderLineOfText(
          renderer, fontSm, objective6Text, textColor, objective6Pos);
