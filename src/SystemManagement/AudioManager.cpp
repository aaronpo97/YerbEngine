...
AudioManager::AudioManager(int const    frequency,
                           ...
                           int const    channels,
                           int const    chunksize)
    : m_frequency(frequency), m_format(format), m_channels(channels),
...
void AudioManager::loadTrack(AudioTrack const track, Path const &filepath) {
...
void AudioManager::loadSample(AudioSample const sample, Path const &filepath) {
...
void AudioManager::playTrack(AudioTrack const track, int const loops) {
...
void AudioManager::playSample(AudioSample const sample, int const loops) {
    if (m_audioSamples[sample]) {
...
void AudioManager::setSampleVolume(AudioSample const sampleTag, int volume) {
    if (volume > MIX_MAX_VOLUME) {
        volume = MIX_MAX_VOLUME;
    }
...
AudioTrack AudioManager::getLastAudioTrack() const { return m_lastAudioTrack; }
...
void AudioManager::toggleMuteAll() {
    bool const anyMuted = m_tracksMuted || m_samplesMuted;
